...
def random_speed_of_sound(depth):
    """Calcula la velocidad del sonido usando la fórmula de Mackenzie."""
    # Rango de temperatura en °C
    temp = np.random.uniform(0, 30) #°C
    # Rango de salinidad en ppt
    salinity = np.random.uniform(30, 40)   # ppt
    ...
    return v, temp, salinity, depth # m/s, °C, ppt, m
...
def propagation_time1(start_position, end_position, depth=None):
    """Calcula el tiempo de propagación acústica entre dos puntos."""
    """
    Calcula el tiempo de propagación de una señal acústica.
    Parámetros:
...
frecuencia_khz = 20 
distancia = 1274.24  # Distancia en metros
...
tiempo1 = propagation_time1(start_position, end_position)
print(f"El tiempo de propagación es otro {tiempo1:.2f} segundos")
...
print(f"Pérdida mixta: {spreading_loss(distancia, MIXED)} dB")


print(f"Perdida total path_loss (spherical) : ", compute_path_loss(frecuencia_khz, distancia, SPHERICAL))
print(f"Perdida total path_loss (cylindrical) : ", compute_path_loss(frecuencia_khz, distancia, CYLINDRICAL))
print(f"Perdida total path_loss (mixta) : ", compute_path_loss(frecuencia_khz, distancia, MIXED))

# Ejemplo de cálculo del rango máximo
loss = 1e-2  # Factor de pérdida permitido
print(f"Distancia máxima alcanzable: {compute_range(frecuencia_khz, loss)} metros")